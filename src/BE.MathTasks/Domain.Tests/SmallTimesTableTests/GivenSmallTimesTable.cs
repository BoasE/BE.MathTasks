...
namespace Domain.Tests.SmallTimesTableTests
{
    public class GivenSmallTimesTable
    {
        ...
        public async Task SecondFactorEqualsTable(int table)
        {
            var task = await GetSut().RandomTask(MultiplicationTaskRequest.ForTable(2));
        ...
        [InlineData(1), InlineData(2), InlineData(3), InlineData(4), InlineData(5),
         InlineData(6), InlineData(7), InlineData(8), InlineData(9), InlineData(10)]
        public async Task GetMultipleRequests(int table)
        {
            var task = await GetSut().RandomTasks(MultiplicationTaskRequest.ForTable(table), 4);
